...
export class SwapManager {
  ...
  async getOptimalGasPrice() {
    try {
      const feeData = await this.provider.getFeeData();
      if (feeData.gasPrice) {
        // Gas price ULTRA-PREMIUM: +300% pour vitesse MAXIMALE
        return Math.floor(Number(feeData.gasPrice) * 4.0).toString();
      }
    } catch (error) {
      // Fallback ultra-élevé pour vitesse maximale
      return '5000000000'; // 5 gwei
    }
    return '4500000000'; // 4.5 gwei par défaut
  }
  ...
  async waitForConfirmationHyperFast(txResponse) {
    const maxAttempts = 60; // 15 secondes max
    const pollInterval = 150; // Polling toutes les 150ms (ULTRA-agressif)
